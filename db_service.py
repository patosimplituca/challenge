...
class BaseDeDatosService:
    ...
    def conectar_bd(self):
        ...
        for db in databases:
            if db[0] == self.config["BaseDeDatos"]["database"]:
                database_exists = True
                break

        # Si la base de datos no existe, crearla
        if not database_exists:
            cursor.execute("CREATE DATABASE {}".format(self.config["BaseDeDatos"]["database"]))
    ...
    def obtener_archivos(self, servicio_drive):
        try:
            resultados = servicio_drive.files().list(
                pageSize=10,
                q="'root' in parents",
                fields="nextPageToken, files(id, name, fileExtension, owners, shared, permissions, modifiedTime)"
            ).execute()
            ...
            if not archivos:
                ...
                for archivo in archivos:
                    ...
                    tipo_archivo = archivo.get('fileExtension', 'Google Document')
